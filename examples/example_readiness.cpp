...
int main() {
  ...
  def.kernel_key = "k";
  def.kernel_generation = engine_residency::KernelGeneration(1);
  def.graph_required = true;
  def.graph_generation = engine_residency::GraphGeneration(1);
  def.compatibility = engine_residency::CompatibilityKey::with_namespace("ref/v1");
  rt.define_engine(def);

  engine_residency::ReadinessProfile prof;
  prof.profile_id = engine_residency::ReadinessProfileId(1);
  prof.profile_generation = engine_residency::ReadinessProfileGeneration(1);
  prof.engine_id = def.engine_id;
  prof.name = "example-profile";
  prof.graph_required = true;
  prof.warmup_required = true;
  prof.compatibility = def.compatibility;
  engine_residency::ProfileRequirement req;
  req.category = engine_residency::ComponentCategory::MODEL;
  req.min_state = engine_residency::ComponentState::VERIFIED;
  req.subject = "m";
