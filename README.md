...
Engine Residency provides a deterministic CPU reference backend and a real CUDA reference engine (RTX 5090 / sm_120) implementing bounded, deterministic serving-like computation with real model-like weight buffers, explicit workspace, KV-like capacity, real kernels, a real executable CUDA graph for a graph-required profile, host-pinned staging, and CPU-reference parity. The multiprocess reference deployment runs independent OS processes (coordinator, Worker A, Worker B, controller/client) over framed loopback TCP with HELLO/REGISTER, explicit registration authorization, bounded checksummed frames, partial read/write correctness, reconnect behavior, and clean shutdown. A targeted worker-command transport routes controller EXECUTE/DRAIN commands to the intended worker session with command-identity/response correlation, current-epoch and worker-boot binding, generation-bound authorization, bounded pending commands, explicit command acceptance/completion, disconnect and stale-session handling, and backend-cleanup acknowledgment. It uses one coherent inbound decoding owner per connection and serializes complete-frame writes; runtime locks are never held across a network/backend wait, and an old command or acknowledgment can never mutate a replacement incarnation. Real-process drain (admitted use held outstanding while a drain is requested, admission fenced, admitted work completing with parity, then worker backend cleanup acknowledged before DRAINED) and real-process make-before-break replacement (distinct old and candidate CUDA worker processes, candidate readiness verified, authorized cutover, old admission fenced, old drained/retired, replacement executing authorized CUDA work with parity) are executed over real coordinator + controller + CUDA worker OS processes, with explicit event barriers rather than arbitrary sleeps. A deterministic reference workload is not a production LLM server. No TensorRT, vLLM, SGLang, ROCm, or other backend integration is claimed; those are not implemented.
...
- The documented CUDA build works. The root cause of an earlier builder failure was that the global strict MSVC `/W4 /WX /permissive-` options were passed bare to nvcc, which interpreted the slash-style `/W4` as a second input file ("single input file required"). These are now scoped to C++ compilation only, and the CUDA target passes them to the host compiler via `-Xcompiler`; the kernel and host CUDA library build and link under a standard CMake+Ninja configuration with CUDA 12.9/nvcc.
- The integrated CUDA worker-death proof is executed over real coordinator + real CUDA worker OS processes: Worker A activates and executes authorized CUDA work with CPU parity; A is killed as a real OS process and fenced; stale A registration/component-publication/readiness/activation/serving-use-release/execution-result traffic is rejected without mutating state; Worker B activates under fresh authority and executes with parity; fresh replacement A-prime prepares fresh process-local state. The engine test proves real CUDA graph replay + CPU parity and exact device-memory baseline reconciliation.
- A real two-process coordinator restart is implemented and fully closed: durable state persist/reload, epoch advance, old-epoch rejection, surviving-worker revalidation, graph-required READY recovery, and post-recovery authorized CUDA execution with CPU parity are all executed and passing.
- x64 MSVC AddressSanitizer is runnable on this host. The VS 2022 Community toolset is missing the x64 ASan runtime, but the VS 2022 BuildTools installation (toolset 14.44.35207) carries the complete x64 `clang_rt.asan_dynamic_runtime_thunk-x86_64.lib` and `clang_rt.asan_dynamic-x86_64.dll`. Building with `-DER_ENABLE_ASAN=ON` under that toolchain produces a working instrumented build; the real instrumented core and transport test suite (core, property, concurrency, adversarial, integration, command) passes under ASan with no findings, including the spawned ASan-instrumented coordinator/worker processes running the full drain/replacement/worker-death proofs over real processes.
- Targeted worker commands, real-process drain, real-process make-before-break replacement, and the hardening regressions (stale epoch/boot rejection, worker death before cleanup acknowledgment with no use leak and no false DRAINED, unsolicited-acknowledgment rejection) are executed by the `er_test_command` suite over real coordinator/controller/worker OS processes.
