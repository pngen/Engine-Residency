...
- A real two-process coordinator restart is implemented (durable state persist/reload + epoch advance + old-epoch rejection) and the coordinator-termination/durable-recovery/epoch-advance/old-epoch-rejection steps are executed and passing. The post-recovery re-authorized worker's readiness currently returns BLOCKED on the graph requirement — a remaining internal replay-eligibility defect that is not yet resolved, so full restart closure is not claimed.
